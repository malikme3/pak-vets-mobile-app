...
export default function AddMediaScreen() {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      setSelectedImageName(result.assets[0].fileName || `photo_${Date.now()}.jpg`);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Presigned URL endpoint not available, using fallback:', presignedError);
        
        // Check if it's a 404 error (endpoint doesn't exist)
        const apiError = presignedError as ApiError;
        const is404 = apiError && typeof apiError === 'object' && 'status' in apiError && apiError.status === 404;
        const is404Message = presignedError instanceof Error && (
          presignedError.message.includes('404') || 
          presignedError.message.includes('Request failed with status code 404')
        );
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content}>
        ...
        <Card style={styles.card}>
          <Text style={[styles.label, { color: colors.text }]}>Select Photo</Text>
        ...
        {selectedImage && (
          <Button
            title={uploading || createMediaMutation.isPending ? "Uploading..." : "Upload Photo"}
